...
When we look back to our boxplots in Data Visualization part, wan can see that there are distinct differences among the age groups and among education groups while disparities among other two categorical predictors are not quite obvious. 

Therefore it is straightforward for us to consider treating grouped variable `age_decades` and `education `as mixed effects, and then we refit the multiple linear regression model above still keeping the variable `age` as a fixed effect for the sake of goodness of fit and better predictive effect of the response mean.

```{python eval=FALSE}
df["group"] = 1 
vcf = {"age_decades": "0 + C(age_decades)", "education": "0 + C(education)"}
mixedlm = sm.MixedLM.from_formula("CVH ~ gender + age + education + insurance", groups="group", vc_formula=vcf, re_formula="0", data=df).fit(reml=False)
...
Again, this addition analysis is just an supplement to our corn analysis. It takes weights into consideration and the corn result is consistent with our final result.  
